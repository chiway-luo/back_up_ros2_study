...
"""
    需求:编写launch文件,启动slam_toolbox的同步建图节点 
    sync_slam_toolbox节点 重视质量 但是延迟较高 适合离线建图
    适用于对实时性要求不高的场景,如事后处理和离线地图构建
...
def generate_launch_description():
    ...
    # param参数化
    ld.add_action(DeclareLaunchArgument('path_param',default_value=my_slam_config_path))
    ...
    slam_toolbox_node = Node(
        ...
        name='slam_toolbox_sync',
        output='screen',
        parameters=[
            LaunchConfiguration('path_param'),
            {'use_sim_time': True}
        ]
    )
