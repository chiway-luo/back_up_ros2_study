...
def generate_launch_description():
    ...
    ld.add_action(DeclareLaunchArgument('car_num',default_value='robot_0'))
    ...
    nav2_launch = IncludeLaunchDescription(
        launch_description_source=PythonLaunchDescriptionSource(
            os.path.join(nav2_pkg, 'launch', 'multi', 'nav2_multi.launch.py')
        ),
        launch_arguments={
            'use_sim_time': LaunchConfiguration('use_sim_time'),
            'car_num': LaunchConfiguration('car_num')
        }.items()
    )
