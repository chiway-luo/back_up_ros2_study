...
public:
    TFListener(std::string str1,std::string str2):Node(str1,str2){
        ...
        // 3-1声明参数服务
        this->declare_parameter("goal_x_offset",0.0);
        this->declare_parameter("goal_y_offset",0.0);
        ...
        goal_x_offset_ = this->get_parameter("goal_x_offset").as_double();
        goal_y_offset_ = this->get_parameter("goal_y_offset").as_double();
        // 3-2创建缓冲区对象
...
private:
    ...
    std::double_t goal_x_offset_;//目标x偏移量
    std::double_t goal_y_offset_;//目标y偏移量
